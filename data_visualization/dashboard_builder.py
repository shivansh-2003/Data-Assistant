...
def get_layout_grid(layout: str) -> tuple:
    """
...
def remove_chart_from_dashboard(chart_id: int):
...
def generate_chart_from_config(df: pd.DataFrame, config: Dict[str, Any]) -> go.Figure:
    """
    ...
    if chart_mode == 'basic':
        ...
        return generate_small_multiples(
            df,
            config.get('x_col'),
            config.get('y_col'),
            config.get('facet_col'),
            config.get('base_chart_type', 'bar')
        )
    elif chart_mode == 'faceted':
        return generate_faceted_chart(
            df,
            config.get('x_col'),
            config.get('y_col'),
            config.get('facet_col'),
            config.get('base_chart_type', 'scatter')
        )
    elif chart_mode == 'layered':
        return generate_layered_chart(
            df,
            config.get('x_col'),
            config.get('y_cols', []),
...
def render_dashboard_tab(df: pd.DataFrame, selected_table: str):
    """
    ...
    if dashboard_active and pinned_count > 0:
        rows, cols = get_layout_grid(dashboard_layout)
            ...
            for col_idx in range(cols):
                with grid_cols[col_idx]:
                    if chart_idx < len(charts):
                        ...
                        # Generate chart
                        try:
                            fig = generate_chart_from_config(df, config)
                            ...
                            with st.expander(f"Chart {chart_idx + 1} Info", expanded=False, key=expander_key):
                                ...
                                if st.button(f"Remove Chart {chart_idx + 1}", key=remove_key):
                                    remove_chart_from_dashboard(chart_entry['id'])
            ...
            with col_exp1:
                try:
                    ...
                    for chart_idx, chart_entry in enumerate(charts):
                        try:
                            fig = generate_chart_from_config(df, chart_entry['config'])
...
def get_current_chart_config(
    ...
    elif chart_mode in ['small_multiples', 'faceted']:
        config.update({
            'facet_col': composition_params.get('facet_col'),
            'base_chart_type': composition_params.get('base_chart_type', 'bar')
        })
    elif chart_mode == 'layered':
        config.update({
            'y_cols': composition_params.get('y_cols', []),
            'layer_types': composition_params.get('layer_types', []),
            'opacity': composition_params.get('opacity', 0.7)
        })
