...
class ResponseFormatter:
    """Formatter for chatbot responses with visualization support."""
    ...
    def format_response(
        ...
        if viz_config:
            try:
                chart_figure = self.generate_chart_if_needed(viz_config, session_id)
        ...
        data_snippets = self.extract_data_snippets(agent_response)
    ...
    def generate_chart_if_needed(
        ...
        try:
            ...
            if not table_name:
                self.logger.warning("No table name provided for chart generation")
                return None
            ...
            if table_name not in dfs:
                self.logger.warning(f"Table '{table_name}' not found in session")
                return None
            ...
            self.logger.info(f"Generated {chart_type} chart for session {session_id}")
            return chart_figure
    ...
    def extract_data_snippets(self, agent_response: str) -> List[Dict[str, Any]]:
        """
