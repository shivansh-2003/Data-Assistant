...
def _handle_summarize_last(state: Dict) -> Dict:
    """Handle 'summarize that' intent: summarize previous result without running code. Returns updated state."""
...
def _execute_plan(plan: list, df_dict: Dict[str, Any], state: Dict) -> Dict:
    """Build unified code from plan steps and execute. Sets state['generated_code']. Returns execution_result dict."""
    ...
    if final_output_var != "result":
        if f"{final_output_var} = " not in unified_code and f"{final_output_var}=" not in unified_code:
            ...
            unified_code = unified_code.replace(f"{final_output_var}=", "result=", 1)
    state["generated_code"] = unified_code
    logger.info("Executing multi-step plan...")
    return execute_pandas_code(unified_code, df_dict)


def _execute_with_llm(query: str, schema: Dict, df_dict: Dict[str, Any], state: Dict) -> Dict:
    """Try rule-based execution; otherwise generate pandas code with LLM and execute. Sets state['generated_code']. Returns execution_result dict."""
    logger.info("Checking if query can be handled by rule-based executor...")
    rule_based_result = try_rule_based_execution(query, df_dict)
    ...
    logger.info(f"Generating pandas code for: {query[:50]}...")
    code = generate_pandas_code(query=query, schema=schema, df_names=list(df_dict.keys()))
    state["generated_code"] = code
    logger.info("Executing pandas code...")
    return execute_pandas_code(code, df_dict)
...
def insight_node(state: Dict) -> Dict:
    ...
    Dispatches to _handle_summarize_last, _execute_plan, or _execute_with_llm; then stores output in state.
    ...
    try:
        ...
        if state.get("intent") == INTENT_SUMMARIZE_LAST:
            return _handle_summarize_last(state)

        plan = state.get("plan")
        if plan and len(plan) > 0:
            execution_result = _execute_plan(plan, df_dict, state)
        else:
            execution_result = _execute_with_llm(insight_query, schema, df_dict, state)
        ...
        if isinstance(output, pd.DataFrame):
            ...
            try:
                ...
                pass
            state["insight_data"] = {"type": "value", "data": output}
