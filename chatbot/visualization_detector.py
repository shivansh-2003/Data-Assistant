...
class VisualizationDetector:
    """Detector for visualization needs and chart parameter extraction."""
    ...
    def detect_visualization_need(self, query: str) -> Tuple[bool, Optional[str]]:
        """
        ...
        for keyword in self.EXPLICIT_VIZ_KEYWORDS:
            if keyword in query_lower:
                # Try to infer chart type from context
                chart_type = self._infer_chart_type_from_query(query_lower)
        ...
        if any(keyword in query_lower for keyword in self.PART_TO_WHOLE_KEYWORDS):
            return True, "pie"
        
        # No visualization needed
        return False, None
    
    def _infer_chart_type_from_query(self, query: str) -> Optional[str]:
    ...
    def get_chart_type_recommendation(self, query: str, column_types: Dict[str, str]) -> str:
        """
        ...
        """
        needs_viz, suggested_type = self.detect_visualization_need(query)
...
_default_detector = VisualizationDetector()
