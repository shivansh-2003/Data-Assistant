...
from .visualization_detector import detect_visualization_need, extract_chart_parameters
...
def create_chatbot_agent(
    ...
    try:
        ...
        logger.warning(f"Agent creation with default params failed, trying alternative: {e}")
        try:
            ...
            logger.warning("Using minimal agent parameters")
            agent = create_pandas_dataframe_agent(
                llm,
                df_list,
                verbose=True,
    ...
    logger.info(f"Created chatbot agent for session {session_id} with {len(df_list)} DataFrames")
    return agent
...
async def process_chat_query(
    ...
    try:
        # 1. Load DataFrames
        logger.info(f"Loading DataFrames for session {session_id}")
        ...
        operation_history = get_operation_history(session_id, streamlit_session_state)
        
        # 3. Detect visualization need
        needs_viz, suggested_chart_type = detect_visualization_need(user_query)
        
        # 4. Create agent (could cache this per session for performance)
        agent = create_chatbot_agent(session_id, dfs)
        ...
        context_prompt = build_context_prompt(session_id, schema, operation_history, conversation_history)
        
        # 7. Invoke agent
        logger.info(f"Invoking agent for query: {user_query[:50]}...")
        agent_response = await invoke_agent(agent, user_query, context_prompt)
        ...
        if needs_viz and dfs:
            ...
            chart_config = extract_chart_parameters(
                user_query,
                first_df,
                first_table_schema,
