...
- **LLM:** OpenAI (e.g. GPT-4o), configurable via env.
...
- **`execution/code_generator.py`:** Uses the code_generator prompt from `system_prompts.py` to produce pandas code. Prompts enforce:
...
All in `prompts/system_prompts.py`:
...
- **`streamlit_ui.py`:** Renders the InsightBot tab: session pill, sidebar (options, quick actions, clear chat), chat history, suggestion chips, and input. Uses `graph.get_state(config)` and invokes the graph with `session_id`, `messages`, schema, table_names, etc. **Message history:** When `response_snapshots` is present, `display_message_history` iterates messages and for each AI message renders its snapshot (table if no chart/viz_error, chart from `viz_config`, code expander). So previous visualizations stay visible when the user sends a new query. Fallback: when there are no snapshots, a single viz/table/code is shown for the last response.
...
├── graph.py              # StateGraph, nodes, edges, checkpointer
...
│   └── safe_executor.py  # Sandboxed execution, timeout
│
├── prompts/
│   ├── __init__.py
│   └── system_prompts.py # Router, analyzer, code_generator, summarizer, etc.
│
└── utils/
    ├── __init__.py
...
1. **Environment:** Set `OPENAI_API_KEY` (and optional `OPENAI_MODEL`, e.g. `gpt-4o`). Ensure Redis and app backend are running if using session data.
2. **Run app:** e.g. `streamlit run app.py`; open the InsightBot (chatbot) tab. Upload data or use an existing session.
...
4. **Clarification:** Ask something that matches multiple columns (e.g. “show sales”) where “sales” matches two columns; confirm “Did you mean …?” and that the next message uses the chosen column.
5. **Correlation:** Ask “show correlation” or “correlation between Price and Weight”; confirm numeric-only behavior and no full-dataframe or groupby `.corr()` errors.
...
- **OpenAI errors:** Check `OPENAI_API_KEY` and quota.
