...
- **🤖 LangGraph-Powered Architecture**: State graph with router, analyzer, insight, viz, responder, clarification, and suggestion nodes; persistent memory via MemorySaver
- **💬 Multi-Turn Conversations**: Context-aware follow-ups (e.g. "What about the maximum?" resolved into full questions); conversation context tracks last columns, aggregation, filters
- **🔍 Column Clarification**: When multiple columns match a term (e.g. "sales"), asks "Did you mean X or Y?" and resolves on the next turn
- **💡 Suggestion Engine**: After each response, generates three contextual follow-up questions shown as clickable chips
- **📊 Per-Turn Visualizations**: Each AI message keeps its own chart/table/code; previous visualizations stay visible when you send new queries (response_snapshots)
- **📋 Report & Summarize**: Supports "give me a report on X" and "summarize that" / "what does that show?" using last result
- **🔧 Function Calling**: Dynamic tool selection (insight_tool, bar_chart, line_chart, scatter_chart, histogram, etc.) from analyzer
...
- **⚡ Safe Code Execution**: Pandas code generated by LLM, executed in sandbox with timeout; correlation uses numeric columns only
- **📈 Charts in Chat**: Plotly charts and data tables embedded per response; fallback to table when chart fails (e.g. too many categories)
- **📝 Query Types Supported**:
  ...
  - Correlations (numeric-only; generic "show correlation" or two-column)
  - Report and summarize_last (re-summarize previous result)
  - Follow-ups that build on previous question
...
│   ├── streamlit_ui.py       # Streamlit UI components (history, snapshots, chips)
│   ├── README.md             # Chatbot architecture and implementation (see here for full detail)
...
│   │   ├── analyzer.py       # Tool selection (function calling)
│   │   ├── insight.py        # Pandas code generation and safe execution
│   │   ├── viz.py            # Chart config and Plotly generation
...
│   │   ├── code_generator.py # LLM pandas code (e.g. numeric-only correlation)
│   │   └── safe_executor.py  # Sandboxed execution with timeout
│   ├── utils/
│   │   └── session_loader.py # Load DataFrames and metadata from Redis
│   ├── prompts/
│   │   └── system_prompts.py # Router, analyzer, code_generator, summarizer, suggestions, etc.
│   └── INSIGHTBOT_IMPLEMENTATION.md  # Implementation checklist and history
│
...
**Architecture**: State graph with router → (clarification | analyzer | insight | responder) → insight → viz | responder → responder → suggestion → END; MemorySaver for persistence; response_snapshots so each AI message keeps its chart/table/code.
...
- `nodes/analyzer.py`: Tool selection via function calling (insight_tool, bar_chart, line_chart, etc.)
...
- `nodes/viz.py`: Chart config validation, Plotly generation via data_visualization; sets viz_error on failure
...
- `tools/simple_charts.py`: Bar, line, scatter, histogram
...
- `execution/safe_executor.py`: Sandboxed execution with timeout

**Utilities**: `utils/session_loader.py`, `prompts/system_prompts.py`, `state.py`, `graph.py`. See `chatbot/README.md` for full schema and flow.
